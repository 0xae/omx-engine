...
static void reply_internal_with_error(nw_ses *ses, int status, char* error)
{
    send_http_response_simple(ses, status, error, 0);
}
...
static void reply_options_ok(nw_ses *ses, http_request_t *request) {
    ...
    http_response_set_header(response, "Access-Control-Allow-Headers", "OPTIONS, GET, PUT, POST, DELETE, HEAD, AUTHORIZATION, Authorization, authorization, Content-Type");
    response->status = 200;

    // send_http_response_simple(ses, 200, reply_str, strlen(reply_str));
    send_http_response(ses, response);
}

static int on_http_request(nw_ses *ses, http_request_t *request) {
    if (request->method == 6) {
        reply_options_ok(ses, request);
        return 0;
    }
